...
BEGIN_EVENT_TABLE(BrushToolPanel, PalettePanel)
	...
	EVT_TOGGLEBUTTON(PALETTE_TERRAIN_REFRESHZONE_TOOL, BrushToolPanel::OnClickRefreshZoneBrushButton)
END_EVENT_TABLE()
...
void BrushToolPanel::LoadAllContents()
{
	...
	if(large_icons) {
		...
		sub_sizer->Add(refreshzoneBrushButton = newd BrushButton(this, g_gui.refresh_brush, RENDER_SIZE_32x32, PALETTE_TERRAIN_REFRESHZONE_TOOL));
		refreshzoneBrushButton->SetToolTip("Refresh Zone Tool");
		...
		ASSERT(g_gui.refresh_brush);
		sub_sizer->Add(refreshzoneBrushButton = newd BrushButton(this, g_gui.refresh_brush, RENDER_SIZE_16x16, PALETTE_TERRAIN_REFRESHZONE_TOOL));
			refreshzoneBrushButton->SetToolTip("Refresh Zone Tool");
	}
...
Brush* BrushToolPanel::GetSelectedBrush() const
{
	...
	if (refreshzoneBrushButton->GetValue())
...
void BrushToolPanel::OnClickRefreshZoneBrushButton(wxCommandEvent & event)
{
	g_gui.ActivatePalette(GetParentPalette());
