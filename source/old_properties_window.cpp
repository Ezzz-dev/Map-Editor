	...
	description_field(nullptr)
{
	...
	if(Container* container = dynamic_cast<Container*>(edit_item)) {
		...
		subsizer->Add(newd wxStaticText(this, wxID_ANY, wxT("Chest Quest Number")));
		chestquestnumber_field = newd wxSpinCtrl(this, wxID_ANY, i2ws(edit_item->chestquestnumber), wxDefaultPosition, wxSize(-1, 20), wxSP_ARROW_KEYS, 0, 0xFFFF, edit_item->chestquestnumber);
		subsizer->Add(chestquestnumber_field, wxSizerFlags(1).Expand());
		...
		subsizer->Add(newd wxStaticText(this, wxID_ANY, wxT("Movement ID")));
		unique_id_field = newd wxSpinCtrl(this, wxID_ANY, i2ws(edit_item->getUniqueID()), wxDefaultPosition, wxSize(-1, 20), wxSP_ARROW_KEYS, 0, 0xFFFF, edit_item->getUniqueID());
		subsizer->Add(unique_id_field, wxSizerFlags(1).Expand());
		...
		*/
		...
		if(door) {
			...
			keyholenumber_field = newd wxSpinCtrl(this, wxID_ANY, i2ws(door->keyholenumber), wxDefaultPosition, wxSize(-1, 20), wxSP_ARROW_KEYS, 0, 0xFFFF, door->keyholenumber);
			subsizer->Add(keyholenumber_field, wxSizerFlags(1).Expand());

			subsizer->Add(newd wxStaticText(this, wxID_ANY, wxT("Door Level")));
			doorlevel_field = newd wxSpinCtrl(this, wxID_ANY, i2ws(door->doorlevel), wxDefaultPosition, wxSize(-1, 20), wxSP_ARROW_KEYS, 0, 0xFFFF, door->doorlevel);
			subsizer->Add(doorlevel_field, wxSizerFlags(1).Expand());

			subsizer->Add(newd wxStaticText(this, wxID_ANY, wxT("Door Quest Number")));
			doorquestnumber_field = newd wxSpinCtrl(this, wxID_ANY, i2ws(door->doorquestnumber), wxDefaultPosition, wxSize(-1, 20), wxSP_ARROW_KEYS, 0, 0xFFFF, door->doorquestnumber);
			...
			doorquestvalue_field = newd wxSpinCtrl(this, wxID_ANY, i2ws(door->doorquestvalue), wxDefaultPosition, wxSize(-1, 20), wxSP_ARROW_KEYS, 0, 0xFFFF, door->doorquestvalue);
			subsizer->Add(doorquestvalue_field, wxSizerFlags(1).Expand());
		}

		if (edit_item->isKey()) {
			subsizer->Add(newd wxStaticText(this, wxID_ANY, wxT("Key Number")));
			keynumber_field = newd wxSpinCtrl(this, wxID_ANY, i2ws(edit_item->keynumber), wxDefaultPosition, wxSize(-1, 20), wxSP_ARROW_KEYS, 0, 0xFFFF, edit_item->keynumber);
...
void OldPropertiesWindow::OnClickOK(wxCommandEvent& WXUNUSED(event))
{
	if(edit_item) {
		if(dynamic_cast<Container*>(edit_item)) {
			...
			int chestnumber = chestquestnumber_field->GetValue();
			...
			edit_item->chestquestnumber = chestnumber;
		}
		else if ((edit_item->canHoldText() || edit_item->canHoldDescription()) && !edit_item->isDoor()) {
			// Book
			int new_uid = unique_id_field->GetValue();
			int new_aid = action_id_field->GetValue();
			...
			if (edit_item->isKey()) {
				int keynumber = keynumber_field->GetValue();
				edit_item->keynumber = keynumber;
			}
			...
			if(door) {
				...
				int keyhole = keyholenumber_field->GetValue();
				int doorlevel = doorlevel_field->GetValue();
				int questnumber = doorquestnumber_field->GetValue();
				int questvalue = doorquestvalue_field->GetValue();

				door->setDoorID(new_door_id);
				door->doorlevel = doorlevel;
				door->doorquestnumber = questnumber;
				door->doorquestvalue = questvalue;
				door->keyholenumber = keyhole;
			}
