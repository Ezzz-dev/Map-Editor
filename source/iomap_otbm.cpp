...
bool Item::readItemAttribute_OTBM(const IOMap& maphandle, OTBM_ItemAttribute attr, BinaryNode* stream)
{
	switch (attr) {
		...
		case OTBM_ATTR_CHESTQUESTNUMBER:
		{
			uint16_t subtype;
			if (!stream->getU16(subtype)) {
				return false;
			}
			chestquestnumber = subtype;
			break;
		}
...
void Item::serializeItemAttributes_OTBM(const IOMap& maphandle, NodeFileWriteHandle& stream) const
{
	...
	if(maphandle.version.otbm >= MAP_OTBM_4) {
		...
		if (keynumber > 0) {
			stream.addU8(OTBM_ATTR_KEYNUMBER);
...
void Door::serializeItemAttributes_OTBM(const IOMap& maphandle, NodeFileWriteHandle& stream) const
{
	...
	if (keyholenumber > 0) {
		...
		stream.addU16(static_cast<uint16_t>(keyholenumber));
	}

	if (doorlevel > 0) {
		stream.addU8(OTBM_ATTR_DOORLEVEL);
		stream.addU16(static_cast<uint16_t>(doorlevel));
	}

	if (doorquestnumber > 0) {
		stream.addU8(OTBM_ATTR_DOORQUESTNUMBER);
		stream.addU16(static_cast<uint16_t>(doorquestnumber));
	}

	if (doorquestvalue > 0) {
		stream.addU8(OTBM_ATTR_DOORQUESTVALUE);
		stream.addU16(static_cast<uint16_t>(doorquestvalue));
	}
}
...
bool Container::serializeItemNode_OTBM(const IOMap& maphandle, NodeFileWriteHandle& file) const
{
	...
	if (chestquestnumber > 0) {
		file.addU8(OTBM_ATTR_CHESTQUESTNUMBER);
		file.addU16(chestquestnumber);
	}
...
bool IOMapOTBM::getVersionInfo(NodeFileReadHandle* f,  MapVersion& out_ver)
{
	...
	out_ver.client = CLIENT_VERSION_770;// ClientVersionID(u32);
	return true;
}
...
bool IOMapOTBM::saveMap(Map& map, NodeFileWriteHandle& f)
{
	...
	f.addNode(0);
	{
		f.addU32(MAP_OTBM_3); // Version

		f.addU16(map.width);
		f.addU16(map.height);

		f.addU32(CLIENT_VERSION_770);
		f.addU32(CLIENT_VERSION_770);
