...
void MapDrawer::BlitItem(int& draw_x, int& draw_y, const Tile* tile, const Item* item, bool ephemeral, int red, int green, int blue, int alpha) {
	...
	if(it.id == 0) {
		...
		glEnable(GL_TEXTURE_2D);
		return;
	} else if(it.id == 451 && !options.ingame) {
		glDisable(GL_TEXTURE_2D);
		glBlitSquare(draw_x, draw_y, red, green, 0, alpha/3*2);
		glEnable(GL_TEXTURE_2D);
		return;
	} else if(it.id == 465 && !options.ingame) {
		glDisable(GL_TEXTURE_2D);
		glBlitSquare(draw_x, draw_y, red, 0, 0, alpha/3*2);
		glEnable(GL_TEXTURE_2D);
		return;
	}
	else if (it.id == 466 && !options.ingame) {
		glDisable(GL_TEXTURE_2D);
		glBlitSquare(draw_x, draw_y, red, 0, 0, alpha / 3 * 2);
		glEnable(GL_TEXTURE_2D);
		return;
	}
 else if (it.id == 467 && !options.ingame) {
	 glDisable(GL_TEXTURE_2D);
	 glBlitSquare(draw_x, draw_y, red, 0, 0, alpha / 3 * 2);
	 glEnable(GL_TEXTURE_2D);
	 return;
 ...
 else if (it.id == 471 && !options.ingame) {
	 glDisable(GL_TEXTURE_2D);
	 glBlitSquare(draw_x, draw_y, red, 0, 0, alpha / 3 * 2);
	 glEnable(GL_TEXTURE_2D);
	 return;
 }
 else if (it.id == 472 && !options.ingame) {
	 glDisable(GL_TEXTURE_2D);
	 glBlitSquare(draw_x, draw_y, red, 0, 0, alpha / 3 * 2);
	 glEnable(GL_TEXTURE_2D);
	 return;
 ...
 else if (it.id == 473 && !options.ingame) {
	 glDisable(GL_TEXTURE_2D);
	 glBlitSquare(draw_x, draw_y, red, 0, 0, alpha / 3 * 2);
	 glEnable(GL_TEXTURE_2D);
	 return;
 }
 else if (it.id == 474 && !options.ingame) {
	 glDisable(GL_TEXTURE_2D);
	 glBlitSquare(draw_x, draw_y, red, 0, 0, alpha / 3 * 2);
	 glEnable(GL_TEXTURE_2D);
	 return;
 }
	else if (it.id == 6322 && !options.ingame) {
		glDisable(GL_TEXTURE_2D);
		glBlitSquare(draw_x, draw_y, red, 0, 0, alpha / 3 * 2);
		glEnable(GL_TEXTURE_2D);
		return;
	}
	else if (it.id == 6323 && !options.ingame) {
		glDisable(GL_TEXTURE_2D);
		glBlitSquare(draw_x, draw_y, red, 0, 0, alpha / 3 * 2);
		glEnable(GL_TEXTURE_2D);
		return;
	...
	if(it.isSplash() || it.isFluidContainer()) {
		...
		else if (color == 3 || color == 4 || color == 7) {
			color = 3;
		}
		else if (color == 9) {
			color = 6;
...
void MapDrawer::WriteTooltip(Item* item, std::ostringstream& stream)
{
	...
	if(unique == 0 && action == 0 && item->chestquestnumber == 0 && 
		item->doorlevel == 0 && item->doorquestnumber == 0 && item->doorquestvalue == 0 &&
		item->keyholenumber == 0 && item->keynumber == 0)
		return;
	...
	if (item->chestquestnumber > 0)
		stream << "quest: " << item->chestquestnumber << "\n";
	if (item->doorlevel > 0)
		stream << "level: " << item->doorlevel << "\n";
	if (item->doorquestnumber > 0 && item->doorquestvalue)
		stream << "quest: " << item->doorquestnumber << " value: " << item->doorquestvalue << "\n";
	if (item->keyholenumber > 0)
		stream << "keyhole: " << item->keyholenumber << "\n";
	if (item->keynumber > 0)
		stream << "key: " << item->keynumber << "\n";
}
